...
export const CalculatorPage: React.FC<CalculatorPageProps> = ({ user, materials, machines, processes, tools, regionCosts, regionCurrencyMap, templates = [], onSave, onSaveDraft, onAutoSaveDraft, onSaveTemplate, onBack, existingCalculation, theme, onNavigate, onHeaderInfoChange, onAddTool, calculations = [] }) => {
  ...
  const handleApplyTemplate = (templateId: string) => {
    ...
    setFormData(prev => ({
        ...
        markups: template.markups || INITIAL_INPUT.markups
    }));
  };
        ...
        {isTemplateModalOpen && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-fade-in">
                <Card className="max-w-md w-full relative">
                    <button onClick={() => setIsTemplateModalOpen(false)} className="absolute top-4 right-4 text-text-muted hover:text-text-primary">
                        <CloseIcon />
                    </button>
                    <h2 className="text-xl font-bold text-primary mb-4">Save Configuration as Template</h2>
                    <p className="text-sm text-text-secondary mb-4">This will save your current setups and markups so you can quickly apply them to other quotes.</p>
                    <Input
                        label="Template Name"
                        name="templateName"
                        value={templateName}
                        onChange={(e) => setTemplateName(e.target.value)}
                        placeholder="e.g. Standard 3-Axis Milling"
                    />
