            ...
            {viewMode !== 'list' ? (
                <MaterialForm
                    ...
                    onSave={(material) => {
                         ...
                         // Intimate user success here (using alert for now)
                            ...
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">
                                    <div className="flex items-center gap-2">
                                        Material Grade
                                        <button onClick={(e) => setActivePopover({ column: 'name', anchorEl: e.currentTarget })}><FilterIcon isActive={!!filters.name} /></button>
                                    </div>
                                </th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">
                                    <div className="flex items-center gap-2">
                                        Material Group
                                        <button onClick={(e) => setActivePopover({ column: 'category', anchorEl: e.currentTarget })}><FilterIcon isActive={!!filters.category?.length} /></button>
                                    </div>
                                </th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">
                                     <div className="flex items-center gap-2">
                                        Sub-Category
                                        <button onClick={(e) => setActivePopover({ column: 'subCategory', anchorEl: e.currentTarget })}><FilterIcon isActive={!!filters.subCategory} /></button>
                                    </div>
                                </th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">
                                     <div className="flex items-center gap-2">
                                        Density
                                        <button onClick={(e) => setActivePopover({ column: 'density', anchorEl: e.currentTarget })}><FilterIcon isActive={!!filters.densitySearch} /></button>
                                    </div>
                                </th>
...
const CategoryFilter: React.FC<{filters: Record<string, any>, onApply: (updates: Record<string, any>) => void, onClear: (keys: string[]) => void, uniqueOptions: string[]}> = ({ filters, onApply, onClear, uniqueOptions }) => {
...
const DensityFilter: React.FC<{filters: Record<string, any>, onApply: (updates: Record<string, any>) => void, onClear: (keys: string[]) => void}> = ({ filters, onApply, onClear }) => {
...
const TextFilter: React.FC<{column: string, filters: Record<string, any>, onApply: (updates: Record<string, any>) => void, onClear: (keys: string[]) => void}> = ({ column, filters, onApply, onClear }) => {
...
const FilterContent: React.FC<{column: string, filters: Record<string, any>, onApply: (updates: Record<string, any>) => void, onClear: (keys: string[]) => void, uniqueOptions?: string[]}> = ({ column, filters, onApply, onClear, uniqueOptions=[] }) => {
    if (column === 'category') {
        return <CategoryFilter filters={filters} onApply={onApply} onClear={onClear} uniqueOptions={uniqueOptions} />;
    }

    if (column === 'density') {
        return <DensityFilter filters={filters} onApply={onApply} onClear={onClear} />;
    }
    
    return <TextFilter column={column} filters={filters} onApply={onApply} onClear={onClear} />;
};