...
export const ProcessForm: React.FC<ProcessFormProps> = ({ process, onSave, onCancel, isSuperAdmin }) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
                ...
                <div className="mt-2 text-xs text-text-muted">
                    <p>Available parameters: {(formData.parameters as unknown as ProcessParameter[]).map(p => p.name).filter(Boolean).join(', ') || 'No parameters defined yet.'}</p>
                    <p className="mt-2">Standard JS `Math` functions supported.</p>
                </div>
            </div>
