...
export const OperationModal: React.FC<OperationModalProps> = ({
    ...
    const handleProcessSelect = (process: Process) => {
        ...
        if (Array.isArray(process.parameters)) {
            ...
            if ('holeDepth' in initialParams) initialParams.holeDepth = 20;
            if ('numberOfHoles' in initialParams) initialParams.numberOfHoles = 1;
            if ('length' in initialParams) initialParams.length = 50;
            if ('diameterStart' in initialParams) initialParams.diameterStart = 50;
            if ('diameterEnd' in initialParams) initialParams.diameterEnd = 45;
            if ('facingDiameter' in initialParams) initialParams.facingDiameter = 50;
            if ('totalFacingDepth' in initialParams) initialParams.totalFacingDepth = 2;
            if ('grooveDepth' in initialParams) initialParams.grooveDepth = 5;
            if ('threadLength' in initialParams) initialParams.threadLength = 20;
            if ('numberOfPasses' in initialParams) initialParams.numberOfPasses = 5;
        }
