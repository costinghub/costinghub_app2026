...
const calculateExpiryDate = (user: SubscriberInfo): string | null => {
    if (user.subscription_expires_on) return user.subscription_expires_on;
    if (user.plan_name === 'Free') {
        const created = safeDate(user.subscribed_on);
        if (created) {
...
const getRowClass = (user: SubscriberInfo): string => {
    ...
    const dateString = calculateExpiryDate(user);
...
export const UserManagementPage: React.FC<UserManagementPageProps> = ({ subscribers, theme, onUpdateUser, onSendRecovery, onSendConfirmation }) => {
    ...
    }, [subscribers, filterStatus, filterPlan, searchTerm]);

    const sortedSubscribers = useMemo(() => {
        const sortableItems = [...filteredSubscribers];
        if (sortConfig !== null) {
    ...
    const handleExportCSV = () => {
        const headers = ["Name", "Email", "Company", "Plan", "Status", "Calculations", "Subscribed On", "Expires On"];
        const rows = sortedSubscribers.map(user => [
            `"${user.name}"`,
            `"${user.email}"`,
            `"${user.company_name || ''}"`,
            `"${user.plan_name || 'Free'}"`,
            ...
            safeDate(user.subscribed_on) ? safeDate(user.subscribed_on)!.toLocaleDateString() : 'N/A',
            safeDate(calculateExpiryDate(user)) ? safeDate(calculateExpiryDate(user))!.toLocaleDateString() : 'N/A'
        ]);
        ...
        link.setAttribute("download", `costinghub_users_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="w-full mx-auto space-y-8 animate-fade-in">
        ...
        <div className="flex justify-between items-center bg-surface p-6 rounded-xl border border-border shadow-sm">
            <div>
                <h2 className="text-2xl font-bold text-primary">User Analytics & Management</h2>
                <p className="text-text-secondary mt-1">Monitor plan telemetry, usage limits, and export data for external analytics.</p>
            </div>
      ...
      <Card>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 mb-6 border-b border-border pb-6">
          <div>
              <h2 className="text-2xl font-bold text-primary">User Directory</h2>
              <span className="text-text-secondary">{filteredSubscribers.length} users found</span>
          </div>
          <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            <input 
                type="text" 
                placeholder="Search name, email, company..." 
                className="w-full md:w-64 bg-background border border-border rounded-md px-3 py-2 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary text-text-primary"
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
            />
            <select 
                className="w-full md:w-auto bg-background border border-border rounded-md px-3 py-2 text-sm focus:outline-none focus:border-primary text-text-primary cursor-pointer"
                value={filterPlan}
            ...
            <tbody className="bg-surface divide-y divide-border">
              {sortedSubscribers.map((user) => (
                <tr key={user.id} className={getRowClass(user)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{safeDate(user.subscribed_on) ? safeDate(user.subscribed_on)!.toLocaleDateString() : 'N/A'}</td>
